...
lammda <- c(xx1[cond,1],rep(xx[1],18),xx1[cond,2:3],rep(xx[1],19))*N;
...
timstart <- Sys.time()
  ...
  for(k in 1:20){
    ...
    A1_tuta <-A_0[2,]-A_grad/A_grad_2
    temp_1 <- THETA_tuta%*%rbind(A_0[1,],A1_tuta,A_0[3:4,])
    A_0[2,] <- A1_tuta
  }
  xx <- 1/(1+exp(-temp_1))
  A_grad_2 <- -colSums(th0*xx*(1-xx)*theta_square[,1])
  A_0[2,]<-soft(A1_tuta,-lammda/A_grad_2,J)
  ...
  for(k in 1:20){
    ...
    A2_tuta <-A_0[3,]-A_grad/A_grad_2;
    A2_tuta[J] <- 0
    temp_1 <- THETA_tuta%*%rbind(A_0[1:2,],A2_tuta,A_0[4,])
    A_0[3,] <- A2_tuta
  }
  xx <- 1/(1+exp(-temp_1))
  A_grad_2 <- -colSums(th0*xx*(1-xx)*theta_square[,1])
  
  A_0[3,] <- c(soft(A2_tuta[-J],(-lammda/A_grad_2)[-J],(J-1)),0)
  temp_1 <- THETA_tuta%*%A_0
  ...
  for(k in 1:20){
    ...
    A3_tuta <-A_0[4,]-A_grad/A_grad_2;
    A3_tuta[(J-1):J] <- 0
    temp_1 <- THETA_tuta%*%rbind(A_0[1:3,],A3_tuta)
    A_0[4,] <- A3_tuta
  }
  xx <- 1/(1+exp(-temp_1))
  A_grad_2 <- -colSums(th0*xx*(1-xx)*theta_square[,3])
